...
void PlayScene::Reset(bool reset)
{
...
{
	SoundManager::Instance().load("../Assets/audio/afford.MP3", "afford", SOUND_MUSIC);
	SoundManager::Instance().setMusicVolume(10);
	SoundManager::Instance().playMusic("afford", -1, 0);

	SoundManager::Instance().load("../Assets/audio/tada.MP3", "tada", SOUND_SFX);
	SoundManager::Instance().setSoundVolume(25);
	SoundManager::Instance().load("../Assets/audio/Walking.MP3", "walk", SOUND_SFX);
	// Set GUI Title
	m_guiTitle = "Play Scene";

	const SDL_Color blue = { 0, 0, 250, 250 };
	m_pInstructions1 = new Label("Press H: Debug View", "Consolas", 20, blue, glm::vec2(155.0f, 10.0f));
	m_pInstructions1->setParent(this);
	addChild(m_pInstructions1);

	m_pInstructions2 = new Label("Press F: Find Shortest Path", "Consolas", 20, blue, glm::vec2(198.0f, 30.0f));
	m_pInstructions2->setParent(this);
	addChild(m_pInstructions2);

	m_pInstructions3 = new Label("Press M: Move Ship", "Consolas", 20, blue, glm::vec2(150.0f, 50.0f));
	m_pInstructions3->setParent(this);
	addChild(m_pInstructions3);

	m_pInstructions4 = new Label("Press R: To Reset", "Consolas", 20, blue, glm::vec2(145.0f, 70.0f));
	m_pInstructions4->setParent(this);
	addChild(m_pInstructions4);

	m_pInstructions5 = new Label("DEBUG ONLY", "Consolas", 20, blue, glm::vec2(600.0f, 10.0f));
	m_pInstructions5->setParent(this);
	addChild(m_pInstructions5);
	
	m_pInstructions6 = new Label("Right Click: To Set OPEN Grid GOAL", "Consolas", 20, blue, glm::vec2(600.0f, 30.0f));
	m_pInstructions6->setParent(this);
	addChild(m_pInstructions6);

	m_pInstructions7 = new Label("Left Click: To Set OPEN Grid START", "Consolas", 20, blue, glm::vec2(600.0f, 50.0f));
	m_pInstructions7->setParent(this);
	addChild(m_pInstructions7);
...
void PlayScene::m_moveShip()
{
	...
	if (moveCounter < m_pPathList.size())
	{
		...
		{
			SoundManager::Instance().playSound("walk", 0);
			moveCounter++;
		}
	}
	else
	{
		SoundManager::Instance().playSound("tada", 0);
		m_shipIsMoving = false;
	}
}
